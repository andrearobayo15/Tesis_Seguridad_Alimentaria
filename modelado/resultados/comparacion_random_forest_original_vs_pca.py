"""
COMPARACIÓN RANDOM FOREST: VARIABLES ORIGINALES VS COMPONENTES PCA
================================================================
Script para comparar el rendimiento de Random Forest usando variables originales
vs Random Forest usando componentes PCA para predicción FIES.
...
def cargar_metricas_modelos():
    """Cargar métricas de ambos modelos Random Forest"""
    print("1. Cargando métricas de modelos Random Forest...")
    
    # Cargar métricas Random Forest original
    try:
        with open('d:/Tesis maestria/Tesis codigo/modelado/resultados/metricas/random_forest_metricas.json', 'r') as f:
            metricas_original = json.load(f)
        print("   Métricas Random Forest original cargadas")
    except FileNotFoundError:
        print("   No se encontraron métricas del modelo Random Forest original")
        return None, None
    
    # Cargar métricas Random Forest PCA
    try:
        ...
        print("   Métricas Random Forest PCA cargadas")
    except FileNotFoundError:
        print("   No se encontraron métricas del modelo Random Forest PCA")
        return None, None
    
    return metricas_original, metricas_pca

def extraer_metricas_validacion(metricas_original, metricas_pca):
    """Extraer métricas de validación cruzada de ambos modelos"""
    ...
    print(f"   Random Forest Original - RMSE: {original_cv['RMSE_mean']:.4f}, R²: {original_cv['R2_mean']:.4f}")
    print(f"   Random Forest PCA - RMSE: {pca_cv['RMSE_mean']:.4f}, R²: {pca_cv['R2_mean']:.4f}")
    
    return original_cv, pca_cv

def comparar_metricas(original_cv, pca_cv):
...
def cargar_predicciones():
    ...
    # Predicciones Random Forest original
    try:
        pred_original = pd.read_csv('d:/Tesis maestria/Tesis codigo/modelado/resultados/predicciones/random_forest_predicciones_2025.csv')
        print(f"   Predicciones RF original: {len(pred_original)} registros")
    except FileNotFoundError:
        print("   No se encontraron predicciones Random Forest original")
        return None, None
    
    # Predicciones Random Forest PCA
    try:
        pred_pca = pd.read_csv('d:/Tesis maestria/Tesis codigo/modelado/resultados/predicciones/random_forest_pca_predicciones_2025.csv')
        print(f"   Predicciones RF PCA: {len(pred_pca)} registros")
    except FileNotFoundError:
        print("   No se encontraron predicciones Random Forest PCA")
        return None, None
    
    return pred_original, pred_pca

def comparar_predicciones(pred_original, pred_pca):
...
def crear_visualizaciones(comparacion_metricas, comparacion_predicciones, pred_original, pred_pca):
    ...
    fig.suptitle('Comparación Random Forest: Variables Originales vs PCA', fontsize=16, fontweight='bold')
    ...
    ax1.bar(x - width/2, original_vals, width, label='RF Original', alpha=0.8)
    ax1.bar(x + width/2, pca_vals, width, label='RF PCA', alpha=0.8)
    ...
    if 'FIES_moderado_grave' in pred_original.columns and 'FIES_moderado_grave' in pred_pca.columns:
        ax2.hist(pred_original['FIES_moderado_grave'], bins=20, alpha=0.6, label='RF Original', density=True)
        ax2.hist(pred_pca['FIES_moderado_grave'], bins=20, alpha=0.6, label='RF PCA', density=True)
    ...
    if 'FIES_grave' in pred_original.columns and 'FIES_grave' in pred_pca.columns:
        ax3.hist(pred_original['FIES_grave'], bins=20, alpha=0.6, label='RF Original', density=True)
        ax3.hist(pred_pca['FIES_grave'], bins=20, alpha=0.6, label='RF PCA', density=True)
    ...
    plt.savefig('d:/Tesis maestria/Tesis codigo/modelado/resultados/comparacion_random_forest_original_vs_pca.png', 
                dpi=300, bbox_inches='tight')
    print("   Visualización guardada: comparacion_random_forest_original_vs_pca.png")
    
    plt.show()

def generar_reporte_final(comparacion_metricas, comparacion_predicciones, metricas_original, metricas_pca):
    """Generar reporte final JSON con conclusiones"""
    ...
    reporte = {
        "titulo": "Comparación Random Forest: Variables Originales vs PCA",
        "fecha_analisis": datetime.now().isoformat(),
        "modelos_comparados": {
            "original": {
                "descripcion": "Random Forest con variables originales",
            ...
            "pca": {
                "descripcion": "Random Forest con 7 componentes PCA",
        ...
        "conclusiones": {
            "rendimiento": f"El modelo Random Forest {'PCA' if mejor_modelo == 'PCA' else 'Original'} muestra mejor rendimiento",
        ...
        "recomendacion_cientifica": {
            ...
            "ventajas_pca": [
                ...
                "Mejor interpretabilidad de componentes principales",
    ...
    # Guardar reporte
    with open('d:/Tesis maestria/Tesis codigo/modelado/resultados/reporte_comparacion_random_forest.json', 'w', encoding='utf-8') as f:
        json.dump(reporte, f, indent=2, ensure_ascii=False)
    
    print("   Reporte final guardado: reporte_comparacion_random_forest.json")
    
    # Mostrar resumen
    print(f"\n   RESUMEN EJECUTIVO:")
    print(f"     Modelo recomendado: Random Forest {mejor_modelo}")
...
def main():
    """Función principal para ejecutar comparación Random Forest"""
    
    print("COMPARACIÓN RANDOM FOREST: VARIABLES ORIGINALES VS COMPONENTES PCA")
    print("=" * 70)
    
    # 1. Cargar métricas
    metricas_original, metricas_pca = cargar_metricas_modelos()
    ...
    original_cv, pca_cv = extraer_metricas_validacion(metricas_original, metricas_pca)
    
    # 3. Comparar métricas
    comparacion_metricas = comparar_metricas(original_cv, pca_cv)
    
    # 4. Cargar y comparar predicciones
    pred_original, pred_pca = cargar_predicciones()
    if pred_original is not None and pred_pca is not None:
        comparacion_predicciones = comparar_predicciones(pred_original, pred_pca)
        
        # 5. Crear visualizaciones
        crear_visualizaciones(comparacion_metricas, comparacion_predicciones, pred_original, pred_pca)
    else:
        comparacion_predicciones = {}
        print("   No se pudieron cargar predicciones para comparación visual")
    
    # 6. Generar reporte final
    reporte = generar_reporte_final(comparacion_metricas, comparacion_predicciones, metricas_original, metricas_pca)
    
    print("\n" + "=" * 70)
    print("COMPARACIÓN RANDOM FOREST COMPLETADA EXITOSAMENTE")
    print("=" * 70)
