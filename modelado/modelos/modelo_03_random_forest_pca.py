"""
MODELO 3: RANDOM FOREST CON COMPONENTES PCA PARA PREDICCIÓN FIES
===============================================================
Implementación de Random Forest usando 7 componentes principales para predecir
variables FIES_moderado_grave y FIES_grave con parámetros optimizados
para datos PCA.
...
class ModeloRandomForestPCA:
    """Modelo Random Forest usando componentes PCA para predicción FIES"""
    
    def __init__(self):
        """Inicializar modelo Random Forest PCA"""
        self.df = None
        self.X_train = None
        self.y_train = None
        self.modelo = None
    ...
    def configurar_modelo(self):
        """Configurar Random Forest con parámetros optimizados para datos PCA"""
        print("\n3. Configurando modelo Random Forest para PCA...")
        
        # Configuración optimizada para componentes PCA
        rf_params = {
            'n_estimators': 200,              # Más árboles para PCA (menos overfitting)
            'max_depth': 10,                  # Profundidad moderada
            'min_samples_split': 5,           # Mínimo para dividir nodo
            'min_samples_leaf': 2,            # Mínimo en hoja
            'max_features': 'sqrt',           # Usar raíz cuadrada de features
        ...
        # Crear modelo base Random Forest
        rf_base = RandomForestRegressor(**rf_params)
        
        # Wrapper para múltiples outputs (FIES_moderado_grave, FIES_grave)
        self.modelo = MultiOutputRegressor(rf_base, n_jobs=-1)
        
        print("   Configuración Random Forest PCA:")
        for param, valor in rf_params.items():
            print(f"     {param}: {valor}")
        
        print("   Justificación para PCA:")
        print("     - Más estimadores (200) para aprovechar dimensionalidad reducida")
        print("     - max_features='sqrt' adaptado a 42 features totales")
        print("     - Profundidad moderada (10) para evitar overfitting")
        print("     - Bootstrap y OOB score para validación interna")
        
        return self
    
    def entrenar_modelo(self):
        """Entrenar modelo Random Forest PCA con validación cruzada temporal"""
        print("\n4. Entrenando modelo Random Forest PCA...")
        ...
        print(f"\n   MÉTRICAS VALIDACIÓN CRUZADA TEMPORAL RF PCA:")
        ...
        # Mostrar importancia de features (solo para Random Forest)
        if hasattr(self.modelo.estimators_[0], 'feature_importances_'):
            importancias = self.modelo.estimators_[0].feature_importances_
            feature_names = self.X_train.columns
            
            # Top 10 features más importantes
            indices_importantes = np.argsort(importancias)[-10:][::-1]
            print(f"\n   TOP 10 FEATURES MÁS IMPORTANTES:")
    ...
    def predecir_2025(self):
        ...
        print("\n5. Generando predicciones FIES 2025 con Random Forest PCA...")
    ...
    def guardar_resultados(self):
        """Guardar modelo, predicciones y métricas Random Forest PCA"""
        print("\n6. Guardando resultados Random Forest PCA...")
        ...
        # 1. Guardar modelo
        with open('d:/Tesis maestria/Tesis codigo/modelado/resultados/modelos/random_forest_pca_modelo.pkl', 'wb') as f:
            pickle.dump(self.modelo, f)
        
        # 2. Guardar predicciones 2025
        self.predicciones_2025.to_csv(
            'd:/Tesis maestria/Tesis codigo/modelado/resultados/predicciones/random_forest_pca_predicciones_2025.csv', 
            index=False
        )
        
        # 3. Guardar métricas
        metricas_completas = {
            'modelo': 'RandomForest_PCA',
            'componentes_pca': 7,
        ...
        with open('d:/Tesis maestria/Tesis codigo/modelado/resultados/metricas/random_forest_pca_metricas.json', 'w') as f:
            json.dump(metricas_completas, f, indent=2, ensure_ascii=False)
        
        print("   Modelo RF PCA guardado: modelos/random_forest_pca_modelo.pkl")
        print("   Predicciones: predicciones/random_forest_pca_predicciones_2025.csv")
        print("   Métricas: metricas/random_forest_pca_metricas.json")
        
        return self
    
    def ejecutar_pipeline_completo(self):
        """Ejecutar pipeline completo del modelo Random Forest PCA"""
...
def main():
    """Función principal para ejecutar modelo Random Forest PCA"""
    
    print("MODELO 3: RANDOM FOREST CON COMPONENTES PCA PARA PREDICCIÓN FIES")
    print("=" * 65)
    print("INICIANDO PIPELINE COMPLETO - MODELO RANDOM FOREST PCA")
    print("=" * 65)
    
    # Ejecutar modelo
    modelo_rf_pca = ModeloRandomForestPCA()
    modelo_rf_pca.ejecutar_pipeline_completo()
    
    print("\n" + "=" * 65)
    print("MODELO RANDOM FOREST PCA COMPLETADO EXITOSAMENTE")
    print("=" * 65)
    
    # Resumen final
    rmse_train = modelo_rf_pca.metricas_entrenamiento['RMSE']
    r2_train = modelo_rf_pca.metricas_entrenamiento['R2']
    rmse_cv = modelo_rf_pca.metricas_cv['RMSE_mean']
    r2_cv = modelo_rf_pca.metricas_cv['R2_mean']
    
    print(f"RESUMEN FINAL:")
    print(f"- Modelo: Random Forest con 7 componentes PCA")
    print(f"- Variables predichas: FIES_moderado_grave, FIES_grave")
    ...
    print(f"- Predicciones 2025: {len(modelo_rf_pca.predicciones_2025)} registros")
    print(f"- Archivos generados: modelo RF PCA, predicciones, métricas")
    
    return modelo_rf_pca
