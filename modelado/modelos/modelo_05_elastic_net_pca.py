"""
MODELO 5: ELASTIC NET CON COMPONENTES PCA PARA PREDICCIÓN FIES
==============================================================
Implementación de Elastic Net usando 7 componentes principales 
...
class ModeloElasticNetPCA:
    """Modelo Elastic Net usando componentes PCA para predicción FIES"""
    
    def __init__(self):
        """Inicializar modelo Elastic Net PCA"""
        self.df = None
        self.X_train = None
        self.y_train = None
        self.scaler_X = None
    ...
    def escalar_datos(self):
        """Escalar datos para Elastic Net (crítico para regularización)"""
        print("\n3. Escalando datos para Elastic Net...")
        ...
        # Escalar targets (y) para mejor convergencia
    ...
    def optimizar_hiperparametros(self):
        """Optimizar hiperparámetros Elastic Net usando GridSearch con validación temporal"""
        print("\n4. Optimizando hiperparámetros Elastic Net...")
        
        # Parámetros a optimizar
        param_grid = {
            'estimator__alpha': [0.001, 0.01, 0.1, 1.0, 10.0],        # Regularización total
            'estimator__l1_ratio': [0.1, 0.3, 0.5, 0.7, 0.9],        # Balance L1/L2
            'estimator__max_iter': [1000, 2000, 3000]                 # Iteraciones máximas
        }
        
        # Modelo base Elastic Net
        elastic_base = ElasticNet(random_state=42)
        modelo_multi = MultiOutputRegressor(elastic_base, n_jobs=-1)
        ...
        print(f"   Combinaciones a probar: {len(param_grid['estimator__alpha']) * len(param_grid['estimator__l1_ratio']) * len(param_grid['estimator__max_iter'])}")
    ...
    def entrenar_modelo_final(self):
        ...
        print("\n5. Entrenando modelo final Elastic Net PCA...")
        ...
        for train_idx, val_idx in tscv.split(self.X_train):
            ...
            elastic_fold = ElasticNet(
                alpha=self.mejor_params['estimator__alpha'],
                l1_ratio=self.mejor_params['estimator__l1_ratio'],
                max_iter=self.mejor_params['estimator__max_iter'],
                random_state=42
            )
            modelo_fold = MultiOutputRegressor(elastic_fold, n_jobs=-1)
        ...
        print(f"\n   MÉTRICAS VALIDACIÓN CRUZADA TEMPORAL ELASTIC NET PCA:")
        ...
        # Mostrar coeficientes más importantes (solo para Elastic Net)
        if hasattr(self.modelo.estimators_[0], 'coef_'):
            coeficientes = self.modelo.estimators_[0].coef_
            feature_names = self.X_train.columns
            
            # Top 10 features más importantes por coeficiente absoluto
            indices_importantes = np.argsort(np.abs(coeficientes))[-10:][::-1]
            print(f"\n   TOP 10 FEATURES MÁS IMPORTANTES (COEFICIENTES):")
            for i, idx in enumerate(indices_importantes):
                print(f"     {i+1}. {feature_names[idx]}: {coeficientes[idx]:.4f}")
        
        return self
    
    def predecir_2025(self):
        """Generar predicciones FIES para 2025 usando componentes PCA"""
        print("\n6. Generando predicciones FIES 2025 con Elastic Net PCA...")
    ...
    def guardar_resultados(self):
        """Guardar modelo, predicciones y métricas Elastic Net PCA"""
        print("\n7. Guardando resultados Elastic Net PCA...")
        ...
        with open('d:/Tesis maestria/Tesis codigo/modelado/resultados/modelos/elastic_net_pca_modelo.pkl', 'wb') as f:
            pickle.dump(modelo_completo, f)
        
        # 2. Guardar predicciones 2025
        self.predicciones_2025.to_csv(
            'd:/Tesis maestria/Tesis codigo/modelado/resultados/predicciones/elastic_net_pca_predicciones_2025.csv', 
            index=False
        )
        
        # 3. Guardar métricas
        metricas_completas = {
            'modelo': 'ElasticNet_PCA',
            'componentes_pca': 7,
        ...
        with open('d:/Tesis maestria/Tesis codigo/modelado/resultados/metricas/elastic_net_pca_metricas.json', 'w') as f:
            json.dump(metricas_completas, f, indent=2, ensure_ascii=False)
        
        print("   Modelo Elastic Net PCA guardado: modelos/elastic_net_pca_modelo.pkl")
        print("   Predicciones: predicciones/elastic_net_pca_predicciones_2025.csv")
        print("   Métricas: metricas/elastic_net_pca_metricas.json")
        
        return self
    
    def ejecutar_pipeline_completo(self):
        """Ejecutar pipeline completo del modelo Elastic Net PCA"""
...
def main():
    """Función principal para ejecutar modelo Elastic Net PCA"""
    
    print("MODELO 5: ELASTIC NET CON COMPONENTES PCA PARA PREDICCIÓN FIES")
    print("=" * 65)
    print("INICIANDO PIPELINE COMPLETO - MODELO ELASTIC NET PCA")
    print("=" * 65)
    
    # Ejecutar modelo
    modelo_elastic_pca = ModeloElasticNetPCA()
    modelo_elastic_pca.ejecutar_pipeline_completo()
    
    print("\n" + "=" * 65)
    print("MODELO ELASTIC NET PCA COMPLETADO EXITOSAMENTE")
    print("=" * 65)
    
    # Resumen final
    rmse_train = modelo_elastic_pca.metricas_entrenamiento['RMSE']
    r2_train = modelo_elastic_pca.metricas_entrenamiento['R2']
    rmse_cv = modelo_elastic_pca.metricas_cv['RMSE_mean']
    r2_cv = modelo_elastic_pca.metricas_cv['R2_mean']
    
    print(f"RESUMEN FINAL:")
    print(f"- Modelo: Elastic Net con 7 componentes PCA")
    print(f"- Variables predichas: FIES_moderado_grave, FIES_grave")
    print(f"- Mejores parámetros: {modelo_elastic_pca.mejor_params}")
    ...
    print(f"- Predicciones 2025: {len(modelo_elastic_pca.predicciones_2025)} registros")
    print(f"- Archivos generados: modelo Elastic Net PCA, predicciones, métricas")
    
    return modelo_elastic_pca
